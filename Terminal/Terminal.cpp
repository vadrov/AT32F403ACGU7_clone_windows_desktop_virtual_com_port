...
static inline void CaptureScreen(void)
{
	...
	/* В пространстве имен jpge для структуры params введена дополнительная переменная,
	* которая позволяет не использовать этот цикл. Перемена местами конпонентов цвета R с B происходит уже
	* непосредственно в программе кодера. Там я тоже прописал соответствующие изменения под введенный параметр.
...
static inline int EncodeToJPEG(int quality)
{
	...
	params.m_exchangeRB = true;	//меняем местами компоненты R c B
